...
import { checkPrice, checkProductName, checkQuantity } from 'utils/validation';
...
export const useAddForm = () => {
  ...
  const [productNameError, setProductNameError] = useState<ValidationError>({
    isError: false,
  });
  const [priceError, setPriceError] = useState<ValidationError>({
    isError: false,
  });
  const [quantityError, setQuantityError] = useState<ValidationError>({
    isError: false,
  });
  const {
    userProfile: {
