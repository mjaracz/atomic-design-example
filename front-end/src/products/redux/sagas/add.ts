import { call, takeLatest, put, select, cancelled } from 'redux-saga/effects'
import { RootState } from '../../../redux/types'
import { postProduct } from '../../../utils.api.fetch/postProduct'

function* sagaAddProduct(action) {
  const abortController = new AbortController()
  try {
    const { jwtToken } = yield select((state: RootState) => state.signIn)
    const data = yield call(postProduct, action.body, jwtToken, abortController.signal)
